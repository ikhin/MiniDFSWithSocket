...
class NameNodeHandler(Handler):
    ...
    def send_to_one(self,sock,request):
        self.sock = sock
        if ('put' or 'put2') in request['action']:
            self.put(request)
        elif ('read' or 'read2') in request['action']:
            self.read(request)
        elif ('fetch' or 'fetch2') in request['action']:
            self.fetch(request)
        elif ('ls' or 'ls2') in request['action']:
            self.ls(request)
        elif 'mkdir' in request['action']:
            self.mkdir(request)

    def recv_from_one(self,sock):
    ...
    def put(self,data):
        ...
        self.last_file_id += 1
        # update file tree
        if 'put2' in data['action']:
            dir = data['action']['put2']['save_path']
            if dir[0] == '/': dir = dir[1:]
            if dir[-1] == '/': dir = dir[:-1]
    ...
    def read(self, data):
        ...
        if file_id not in self.id_file_map:
            ...
            if space_left_in_chunk < read_count:
                ...
                data['action']['read']['chunk']= CHUNK_PATTERN % (file_id, start_chunk)
                data['action']['read']['offset'] = read_offset - start_chunk * CHUNK_SIZE
    ...
    def fetch(self,data):
        ...
        # find file_id
        file_dir = data['file']['dir']
        if "fetch2" in data['action']:
            file_id = self.tree.get_id_by_path(file_dir, self.id_file_map)
            data['file']['id'] = file_id
            if file_id < 0:
                data['action']['fetch']['chunks'] = -1
                print('No such file:', file_dir)
                return None
        ...
        if file_id not in self.id_file_map:
            ...
            data['action']['fetch']['chunks']= len(file_chunks)
            # get file's data server
            for chunk in file_chunks:
                data['action']['fetch']['servers'].append(self.chunk_server_map[chunk][0])
            self.send(self.sock, data)
            return True
        return None
