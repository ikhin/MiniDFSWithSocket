        ...
        with open(json_data['file']['path'], 'r') as f_in:
            for item in json_data['server_chunks'][self.server_id]:
                ...
                with open(data_node_dir + os.path.sep + chunk, 'w') as f_out:
                    f_out.write(content)
                    f_out.flush()
        return {'action':'save','done':1}

    def read(self,json_data):
        """read chunk according to offset and count"""
        read_path = (DATA_NODE_DIR % (self.server_id,)) + os.path.sep + json_data['action']['read']['chunk']
        with open(read_path, 'r') as f_in:
            f_in.seek(json_data['action']['read']['offset'])
            content = f_in.read(json_data['action']['read']['count'])
            print(content)
        return {'action':'read','done':1}